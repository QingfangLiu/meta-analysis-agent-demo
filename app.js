(function () {
  ...
  const steps = [
    ...
    { id: "step-4", label: "Planning", title: "Stage 4: Outcome and review planning" },
  ...
  const COCHRANE_REFERENCE_STATUS_ORDER = [
    "matched_analysis",
  ...
  const STRUCTURED_STUDY_CHARACTERISTIC_FIELDS = {
    participant_flow: {
      className: "participant-flow-cell",
      order: [
        "recruited_or_enrolled",
        "randomized_or_allocated",
        "analyzed",
        "lost_to_follow_up_or_dropout",
        "notes",
      ],
      labels: {
        recruited_or_enrolled: "Recruited/enrolled",
        randomized_or_allocated: "Randomized/allocated",
        analyzed: "Analyzed",
        lost_to_follow_up_or_dropout: "Lost to follow-up/dropout",
        notes: "Notes",
      },
    },
    follow_up: {
      className: "follow-up-cell",
      order: ["planned", "actual_or_median", "unit", "notes"],
      labels: {
        planned: "Planned",
        actual_or_median: "Actual/median",
        unit: "Unit",
        notes: "Notes",
      },
    },
    study_arms: {
      className: "study-group-cell",
      order: ["arm_label", "description", "dose_or_intensity", "duration_or_cycles", "n_assigned_or_analyzed", "notes"],
      labels: {
        arm_label: "Arm",
        description: "Description",
        dose_or_intensity: "Dose/intensity",
        duration_or_cycles: "Duration/cycles",
        n_assigned_or_analyzed: "N assigned/analyzed",
        notes: "Notes",
      },
    },
    intervention_groups: {
      className: "study-group-cell",
      order: ["group_label", "description", "dose_or_intensity", "duration_or_cycles", "n_assigned_or_analyzed"],
      labels: {
        group_label: "Group",
        description: "Description",
        dose_or_intensity: "Dose/intensity",
  ...
  const RUN_TIMING_STAGE_LABELS = {
    ...
    post_screening: "Outcome and review planning",
  ...
  let currentEvaluationVisible = true;
  ...
  function restoreDemoUiState() {
    ...
    currentEvaluationVisible = saved.currentEvaluationVisible !== false;
    currentOutcomeBenchmarkView = saved.currentOutcomeBenchmarkView || currentOutcomeBenchmarkView;
    currentCochraneReferenceStatusPlots = new Set(
      Array.isArray(saved.currentCochraneReferenceStatusPlots) ? saved.currentCochraneReferenceStatusPlots : []
    );
    currentCochraneForestPlotViews = new Map(
      Object.entries(saved.currentCochraneForestPlotViews || {})
    );
    currentCochraneStudySelections = new Map(
      Object.entries(saved.currentCochraneStudySelections || {})
    );
  ...
  function renderQueryHistory(queryReview, screeningQueryUpdate, searchMetrics = {}) {
    ...
    if (!entries.length) {
      return `<div class="panel">${renderQueryBlocks("")}</div>`;
    }
  ...
  function renderEvaluationSummary(searchMetrics, outcomeAlignment, comparisonAlignment, ciOverlapArtifact, synthesisDisplayContext = {}) {
    const hasAnyEvaluation = [searchMetrics, outcomeAlignment, comparisonAlignment, ciOverlapArtifact].some(
  ...
  function runTimingSection(timing, runSummary) {
    ...
    if ((!Number.isFinite(totalSeconds) || totalSeconds <= 0) && !stages.length) {
      return `
        <div class="panel run-timing-panel">
          <h3>Run Timing</h3>
  ...
  function extractionPubInfoCell(row) {
    const authorYear = [
      String(row?.first_author_last_name || "").trim(),
      String(row?.year || "").trim(),
    ].filter(Boolean).join(", ");

    return `
      <div class="screen-study-cell">
  ...
  function publicationLinkageSummaryItems(publicationLinkageArtifact, publicationLinkageEvidenceArtifact = {}) {
    const { linkage, evidence } = publicationLinkagePayload(publicationLinkageArtifact, publicationLinkageEvidenceArtifact);
    const groups = Array.isArray(linkage.publication_groups) ? linkage.publication_groups.filter(Boolean) : [];
    const singletonPmids = Array.isArray(linkage.singleton_pmids) ? linkage.singleton_pmids.filter(Boolean) : [];
    const uncertainEntries = Array.isArray(linkage.uncertain_linkage_pmids)
    ...
    groups.forEach((group, index) => {
      ...
      pmids.forEach((pmid) => linkedPmids.add(pmid));
      items.push({
        detailKey: `group-${index}`,
        label: group.group_id || `Group ${index + 1}`,
        count: pmids.length,
        color: LINKAGE_COLORS[items.length % LINKAGE_COLORS.length],
      });
    });

    const uncertainPmids = [];
    const seenUncertain = new Set();
    uncertainEntries.map(normalizeUncertainLinkageItem).forEach((item) => {
      if (!item.pmid || linkedPmids.has(item.pmid) || seenUncertain.has(item.pmid)) {
        return;
      }
      seenUncertain.add(item.pmid);
      uncertainPmids.push(item.pmid);
    });
    if (uncertainPmids.length) {
      items.push({
        detailKey: "uncertain",
        label: "Uncertain linkage",
        count: uncertainPmids.length,
        color: LINKAGE_COLORS[items.length % LINKAGE_COLORS.length],
      });
    }

    const singletonCount = uniquePmids(singletonPmids)
      .filter((pmid) => !linkedPmids.has(pmid) && !seenUncertain.has(pmid))
      .length;
    if (singletonCount) {
      items.push({
        detailKey: "standalone",
  ...
  function renderPublicationLinkageSideKey(publicationLinkageArtifact, publicationLinkageEvidenceArtifact = {}) {
    const { linkage, evidence } = publicationLinkagePayload(publicationLinkageArtifact, publicationLinkageEvidenceArtifact);
    const items = publicationLinkageSummaryItems(publicationLinkageArtifact, publicationLinkageEvidenceArtifact);
    ...
    const slices = items.map((item) => {
      ...
      const slice = `
        ...
        >
          <title>${escapeHtml(item.label)}: ${number(item.count)} studies</title>
        </circle>
      `;
      offset += length;
      return slice;
    ...
    return `
      <details class="linkage-side-key" aria-label="Publication linkage key">
        <summary class="linkage-side-summary">
          <span class="linkage-side-title">Publication Linkage</span>
          <span class="linkage-side-summary-count mono">${number(total)}</span>
        </summary>
        <div class="linkage-side-body">
          <div class="linkage-side-donut-wrap" aria-label="Publication linkage study counts">
          ...
          ${uniqueTotal && uniqueTotal !== total ? `<div class="linkage-side-unique mono">${number(uniqueTotal)} unique records</div>` : ""}
          <div class="linkage-side-legend">
            ${items.map((item) => `
              <div class="linkage-side-row">
                <span class="linkage-legend-swatch" style="background:${item.color}"></span>
                <span class="linkage-side-label">${escapeHtml(item.label)}</span>
                <span class="linkage-side-count mono">${number(item.count)}</span>
              </div>
            `).join("")}
          </div>
        </div>
  ...
  function cochraneReferenceStatusesPresent(ciOverlapArtifact, activePlotKeys = null) {
    const present = new Set();
    if (!Array.isArray(ciOverlapArtifact?.plot_alignments)) {
      return present;
    }
    ciOverlapArtifact.plot_alignments.forEach((plot) => {
      const plotKey = String(plot?.agent_plot_key || "").trim();
      if (activePlotKeys && !activePlotKeys.has(plotKey)) {
        return;
      }
      (plot?.agent_study_reference_statuses || []).forEach((row) => {
        const status = String(row?.cochrane_reference_status || "").trim();
        if (status) {
          present.add(status);
        }
      });
  ...
  function renderCochraneReferenceStatusSideKey(ciOverlapArtifact) {
    if (!currentEvaluationVisible) {
      return "";
    }
    ...
    const presentStatuses = cochraneReferenceStatusesPresent(
      ciOverlapArtifact,
      currentCochraneReferenceStatusPlots
    );
    if (!presentStatuses.size) {
      return "";
    }
    return `
      <details class="cochrane-status-side-key" open aria-label="Cochrane row status color key">
        <summary class="cochrane-status-side-summary">
          <span class="linkage-side-title">Cochrane Row Status</span>
        </summary>
        <div class="cochrane-status-side-body">
          ${COCHRANE_REFERENCE_STATUS_ORDER.filter((status) => presentStatuses.has(status)).map((status) => {
            const meta = COCHRANE_REFERENCE_STATUS_META[status] || {};
            return `
              <div class="cochrane-status-side-row">
                <span class="cochrane-status-side-swatch cochrane-status-side-swatch-${escapeHtml(status)}" style="--status-color:${escapeHtml(meta.color || "#64748b")}"></span>
                <span class="cochrane-status-side-text">
                  <span class="cochrane-status-side-label">${escapeHtml(meta.label || cochraneReferenceStatusSummaryLabel(status))}</span>
                  <span class="cochrane-status-side-detail">${escapeHtml(meta.detail || "")}</span>
                </span>
              </div>
            `;
          }).join("")}
  ...
  function renderLeftSideKeys(publicationLinkageArtifact, publicationLinkageEvidenceArtifact, ciOverlapArtifact) {
    const content = [
      renderPublicationLinkageSideKey(publicationLinkageArtifact, publicationLinkageEvidenceArtifact),
      renderCochraneReferenceStatusSideKey(ciOverlapArtifact),
    ].filter(Boolean).join("");
    return content ? `<div class="side-key-stack">${content}</div>` : "";
  }

  function robFrameworkReference(robFramework) {
    const frameworkId = String(robFramework?.framework_id || "").trim().toLowerCase();
    if (frameworkId === "rob2_rct") {
      return `<sup><a class="sup-link" href="https://sites.google.com/site/riskofbiastool/welcome/rob-2-0-tool" target="_blank" rel="noopener noreferrer">[about]</a></sup>`;
    }
    if (frameworkId === "jbi_prevalence") {
      return `<sup><a class="sup-link" href="https://jbi.global/critical-appraisal-tools" target="_blank" rel="noopener noreferrer">[about]</a></sup>`;
    }
    return "";
  }
  ...
  function robToolSourceSection(robDisplay) {
    const sources = Array.isArray((robDisplay || {}).tool_sources) ? robDisplay.tool_sources : [];
    if (!sources.length) {
      return `
        <details class="detail-card collapsible-table-panel rob-source-panel">
          <summary class="collapsible-table-summary">
            <h3>RoB Tool Sources &amp; Access</h3>
          </summary>
          <p class="note">No local RoB tool source metadata was found in this demo bundle. Rebuild the demo data with the current <span class="mono">demo/build_demo.py</span> to populate this table.</p>
        </details>
      `;
    }
    ...
    function sourceLink(source) {
      ...
      const docs = uniqueTextList(source.local_source_documents || []);
      const urlHtml = url
        ? `<a href="${escapeHtml(url)}" target="_blank" rel="noreferrer">Official source</a>`
        : `<span class="muted">No source URL</span>`;
      const docsHtml = docs.length
        ? `<div class="rob-source-docs">${docs.map((doc) => `<span class="mono">${escapeHtml(doc)}</span>`).join("<br>")}</div>`
        : "";
      return `${urlHtml}${docsHtml}`;
    }

    function sourceLabel(source) {
      return cleanText(source.tool_name) || cleanText(source.tool_id) || "RoB tool";
    }
    ...
    return `
      <details class="detail-card collapsible-table-panel rob-source-panel">
        <summary class="collapsible-table-summary">
          <h3>RoB Tool Sources &amp; Access</h3>
          <span class="collapsible-table-count mono">${number(sources.length)} tools</span>
        </summary>
        <p class="note">This table is loaded from local <span class="mono">source.json</span> files under <span class="mono">templates/rob_tools/</span>. The demo shows native short labels, implementation status, and access notes; full forms and long guidance remain in the official/local source documents.</p>
        <div class="table-wrap screening-wrap">
          <table class="screening-table rob-source-table">
            <thead>
              <tr>
                <th>Tool</th>
                <th>Study / Result Type</th>
                <th>Local Template</th>
                <th>Native Labels</th>
                <th>Copyright / Access</th>
                <th>Source</th>
              </tr>
            </thead>
            <tbody>
              ${sources.map((source) => {
                const judgmentLabels = uniqueTextList(source.native_judgment_labels || []);
                const nativeLabels = uniqueTextList(source.native_labels || []);
                const labelParts = [
                  judgmentLabels.length ? `<strong>Judgments</strong><br>${limitedTextList(judgmentLabels, 5)}` : "",
                  nativeLabels.length ? `<strong>Domains/items</strong><br>${limitedTextList(nativeLabels, 5)}` : "",
                ].filter(Boolean).join("<div class=\"rob-source-label-gap\"></div>");
                return `
                  <tr>
                    <td class="rob-source-tool-cell">
                      <div class="screen-study-primary">${sentence(sourceLabel(source))}</div>
                      <div class="screen-study-secondary mono">${escapeHtml(cleanText(source.tool_id) || "—")}</div>
                    </td>
                    <td class="rob-source-type-cell">${sentence(cleanText(source.study_or_result_type) || "Not specified")}</td>
                    <td>${source.has_local_template ? `<span class="screen-cell pass">Implemented</span>` : `<span class="screen-cell unclear">Source only</span>`}</td>
                    <td class="rob-source-label-cell">${labelParts || "—"}</td>
                    <td class="rob-source-access-cell">
                      ${source.copyright_concern ? `<div>${sentence(source.copyright_concern)}</div>` : ""}
                      ${source.copyright_or_license_note ? `<div class="screen-study-secondary">${sentence(source.copyright_or_license_note)}</div>` : ""}
                    </td>
                    <td class="rob-source-link-cell">${sourceLink(source)}</td>
                  </tr>
                `;
              }).join("")}
            </tbody>
  ...
  function studyTypeRoutingSection(robDisplay) {
    ...
    if (!rows.length) {
      return `
        <details class="detail-card collapsible-table-panel rob-routing-panel">
          <summary class="collapsible-table-summary">
            <h3>Study Type Routing</h3>
          </summary>
    ...
    return `
      <details class="detail-card collapsible-table-panel rob-routing-panel" open>
        <summary class="collapsible-table-summary">
          <h3>Study Type Routing</h3>
    ...
    return `
      <div class="rob-tool-group-list">
        ${groups.map((group) => {
          ...
          return `
            <details class="collapsible-table-panel rob-tool-group" open>
              <summary class="collapsible-table-summary">
  ...
  function riskOfBiasTemplateSection(robFramework) {
    const domains = Array.isArray(robFramework.domains) ? robFramework.domains : [];
    const frameworkLabel = robFramework.framework_label || robFramework.framework_name || "RoB framework";
    const frameworkRef = robFrameworkReference(robFramework);
    const frameworkTitle = sentence(robFramework.framework_name || robFramework.framework_label || "Risk-of-Bias Framework");

    if (!domains.length) {
      return `
        <details class="detail-card collapsible-table-panel rob-template-panel">
          <summary class="collapsible-table-summary rob-template-summary">
            <h3>${frameworkTitle}</h3>
          </summary>
          <p class="note">No review-level RoB template was found for this run.</p>
        </details>
      `;
    }
    ...
    return `
      <details class="detail-card collapsible-table-panel rob-template-panel">
        ...
        <p class="note">This run uses ${sentence(frameworkLabel)} ${frameworkRef}</p>
        <div class="rob-template-list">
          ${domains.map((domain, index) => `
            <div class="rob-template-card">
              <div class="rob-template-index mono">D${index + 1}</div>
              <div class="rob-template-body">
                <div class="rob-template-name">${sentence(domain.name || "Unnamed domain")}</div>
                <div class="rob-template-text">${sentence(domain.description || domain.guidance || domain.prompt || "No domain description provided.")}</div>
              </div>
            </div>
          `).join("")}
        </div>
  ...
  function publicationLinkageSection(publicationLinkageArtifact, publicationLinkageEvidenceArtifact, screenedStudies) {
    ...
    if (!hasContent) {
      return `
        <details class="detail-card publication-linkage-section" style="margin-top:14px;">
  ...
  function outcomesSection(
    ...
    function sourceTagLabel(tag) {
      const labels = {
        candidate_map_derived: "Candidate maps",
        nct_derived: "NCT",
        fulltext_candidate_derived: "Full text",
      };
    ...
    function sourceData() {
      const candidateMap = sourceInventory.candidate_map || sourceInventory.title_abstract_screening || {};
      const nct = sourceInventory.nct || {};
      const fulltext = sourceInventory.fulltext_candidate || {};

      return [
        {
          key: "candidate_map",
          classKey: "title-abstract",
          title: "Candidate Study Maps",
          subtitle: "Outcome candidates identified while mapping screened studies before final outcome planning.",
          outcomes: Array.isArray(candidateMap.outcomes) ? candidateMap.outcomes : [],
          counts: candidateMap.counts || {},
          inputLabel: "signals",
          sourceOutcomeCount: (candidateMap.counts || {}).n_unique_labels,
        },
        {
          key: "nct",
          classKey: "nct",
    ...
    function renderSourceCard(source) {
      ...
      const inputCount = source.inputCount ?? counts.n_outcome_signals ?? source.outcomes.length;
      ...
      return `
        <div class="outcome-source-card outcome-source-${escapeHtml(source.classKey || source.key)}">
          <div class="outcome-source-head">
            <div>
          ...
          ${hasContribution ? `
            <div class="outcome-source-contribution-note">
              Source-only final outcomes: ${compactLabelList((sourceSummary.unique_final_outcomes || []).map((item) => item.name), 4)}; source-only Cochrane analyzed: ${compactLabelList((sourceSummary.unique_cochrane_analyzed_matches || []).map((item) => item.label), 4)}; Cochrane planned in source: ${number(plannedMatched)}/${number(plannedTotal)}.
            </div>
          ` : ""}
          <div class="outcome-source-labels">${escapeHtml(compactLabelList(labels, 9))}</div>
        </div>
      `;
    }
    ...
    return `
      <div class="detail-card outcome-decision-section" style="margin-top:14px;">
        <h3>Outcomes</h3>
  ...
  function subgroupPlanSection(subgroupPlanArtifact) {
    ...
    if (!dimensions.length) {
      ...
      return `
        <details class="detail-card subgroup-plan-section" style="margin-top:14px;">
      ...
      return `
        <div class="outcome-panel subgroup-plan-panel">
          <div class="outcome-panel-head subgroup-plan-head">
            <div>
              <div class="insight-title">Subgroup Dimension ${index + 1}</div>
  ...
  function extractionOverviewSection(overview) {
  ...
  function nctLinkageSection(nctRows) {
    ...
    return `
      <details class="nct-linkage-panel">
        <summary class="collapsible-table-summary">
          <h4>NCT Linkage</h4>
  ...
  function fulltextEligibilitySection(perStudyOutputs) {
    ...
    function eligibilityDecisionBadge(decision, status) {
      const normalized = cleanText(decision).toLowerCase();
      if (normalized === "include") {
        return `<span class="screen-cell pass">Include</span>`;
      }
      if (normalized === "exclude") {
        return `<span class="screen-cell fail">Exclude</span>`;
      }
      const label = cleanText(status).toLowerCase() === "skipped" ? "Not screened" : "No decision";
      return `<span class="screen-cell unclear">${escapeHtml(label)}</span>`;
    }

    function titleAbstractBadge(decision) {
      const normalized = cleanText(decision).toLowerCase();
      if (normalized === "include") {
        return `<span class="screen-cell pass">Include</span>`;
      }
      ...
      if (normalized === "not enough info") {
        return `<span class="screen-cell unclear">Not enough info</span>`;
      }
      return "—";
    }

    function countCriterionJudgments(items, criterionType) {
    ...
    function criterionCountLine(label, items, criterionType) {
      const list = Array.isArray(items) ? items : [];
      if (!list.length) {
        return "";
      }
      const counts = countCriterionJudgments(list, criterionType);
      const countText = Object.entries(counts)
        .map(([key, value]) => `${key}: ${value}`)
        .join("; ");
      return `<div><strong>${escapeHtml(label)}</strong>: ${escapeHtml(countText)}</div>`;
    }

    function criterionTooltip(items, criterionType, heading) {
      const list = Array.isArray(items) ? items : [];
      if (!list.length) {
        return [];
      }
      return [
        `${heading}:`,
        ...list.map((item, index) => {
          const criterion = cleanText(item?.criterion) || `Criterion ${index + 1}`;
          const judgment = eligibilityJudgmentMeta(item?.judgment, criterionType).label;
          const justification = cleanText(item?.justification);
          return `- ${criterion}: ${judgment}${justification ? ` - ${justification}` : ""}`;
        }),
      ];
    }

    function criteriaCell(eligibility) {
      const inclusion = Array.isArray(eligibility?.inclusion_criteria_assessment)
        ? eligibility.inclusion_criteria_assessment
        : [];
      const exclusion = Array.isArray(eligibility?.exclusion_criteria_assessment)
        ? eligibility.exclusion_criteria_assessment
        : [];
      const lines = [
        criterionCountLine("Inclusion", inclusion, "inclusion"),
        criterionCountLine("Exclusion", exclusion, "exclusion"),
      ].filter(Boolean);
      if (!lines.length) {
        return "—";
      }
      const tooltip = [
        ...criterionTooltip(inclusion, "inclusion", "Inclusion criteria"),
        ...criterionTooltip(exclusion, "exclusion", "Exclusion criteria"),
      ].join("\n");
      return `<div class="fulltext-criteria-cell" title="${escapeHtml(tooltip)}">${lines.join("")}</div>`;
    }

    function fulltextMethodCell(source, eligibility) {
      const method = cleanText(source?.method)
        || cleanText(eligibility?.screening_source)
        || cleanText(source?.extraction_text_source)
        || cleanText(source?.skip_reason);
      const pmcid = cleanText(source?.pmcid);
      const inPmc = source?.in_pmc === true ? "In PMC" : source?.in_pmc === false ? "Not in PMC" : "";
      return [
        method ? sentence(sourceLabel(method)) : "",
        pmcid ? `<span class="mono">${escapeHtml(pmcid)}</span>` : "",
        inPmc ? `<span class="screen-study-secondary">${escapeHtml(inPmc)}</span>` : "",
      ].filter(Boolean).join("<br>") || "—";
    }
    ...
    const rows = entries.map((entry) => {
      ...
      return {
        ...
        titleAbstractDecision: cleanText(eligibility.title_abstract_decision || metadata.screen_decision),
    ...
    const included = rows.filter((row) => row.decision.toLowerCase() === "include").length;
    const excluded = rows.filter((row) => row.decision.toLowerCase() === "exclude").length;

    return `
      <details class="collapsible-table-panel fulltext-eligibility-panel" open>
        <summary class="collapsible-table-summary">
          <h4>Full-text Eligibility Gate</h4>
          <span class="collapsible-table-count mono">${number(completed)} screened</span>
        </summary>
        <p class="note">Full-text eligibility is the second screening gate. It runs only when full text is available; <span class="mono">include</span> continues to extraction and RoB, while <span class="mono">exclude</span> stops both. Title/abstract <span class="mono">not enough info</span> candidates can still proceed when no full text was available.</p>
        <div class="stats-grid fulltext-eligibility-stats">
          <div class="stat-card screening-summary-card screening-summary-include">
            <div class="stat-label">Full-text include</div>
            <div class="stat-value">${number(included)}</div>
          </div>
          <div class="stat-card screening-summary-card screening-summary-exclude">
            <div class="stat-label">Full-text exclude</div>
            <div class="stat-value">${number(excluded)}</div>
          </div>
          <div class="stat-card screening-summary-card screening-summary-unclear">
            <div class="stat-label">Not full-text screened</div>
            <div class="stat-value">${number(rows.length - completed)}</div>
          </div>
        </div>
        <div class="table-wrap screening-wrap">
          <table class="screening-table fulltext-eligibility-table">
            <thead>
              <tr>
                <th>#</th>
                <th>PMID</th>
            ...
            <tbody>
              ${rows.map((row, index) => `
                <tr>
                  ...
                  <td class="screen-col-title">${extractionPubInfoCell(row)}</td>
                  <td>${yesNo(row.source.fulltext_obtained === true)}</td>
                  <td class="fulltext-method-cell">${fulltextMethodCell(row.source, row.eligibility)}</td>
                  <td>${eligibilityDecisionBadge(row.decision, row.status)}</td>
                  <td>${titleAbstractBadge(row.titleAbstractDecision)}</td>
                  <td>${criteriaCell(row.eligibility)}</td>
                  <td class="fulltext-eligibility-reason">${row.reason ? sentence(row.reason) : "—"}</td>
                </tr>
              `).join("")}
            </tbody>
          </table>
  ...
  function extractionAttemptSection(
    ...
    return `
      ${extractionOverviewSection(extractionOverview)}
      <div class="outcome-panel-list">
  ...
  function studyCharacteristicsSection(studyCharacteristicsSummary) {
    const summary = studyCharacteristicsSummary || {};
    const rows = Array.isArray(summary.studies) ? summary.studies : [];

    function rowCountText() {
      const nStudies = summary.n_studies ?? rows.length;
      return `${number(nStudies)} candidate studies`;
    }

    if (!rows.length) {
      return `
        <details class="collapsible-table-panel study-characteristics-panel">
          <summary class="collapsible-table-summary study-characteristics-summary">
            <h3>Study Characteristics</h3>
          </summary>
          <p class="note">No study-characteristics summary rows were found for this run.</p>
        </details>
      `;
    }
    ...
    const metadataFields = new Set([
      ...
      "text_source_used",
      "non_extraction_reason",
      "field_evidence_json",
    ]);
    const displayFields = [];
    rows.forEach((row) => {
      Object.entries(row || {}).forEach(([field, value]) => {
        if (metadataFields.has(field) || displayFields.includes(field) || !hasValue(value)) {
          return;
        }
        displayFields.push(field);
      });
    });
    const scrollKey = "study-characteristics";
    ...
    function yesNo(value) {
      ...
      return hasValue(value) ? sentence(value) : "—";
    }

    function renderValueCell(row, field) {
      const value = row[field];
      const evidenceByField = parseFieldEvidence(row);
      const tooltip = fieldEvidenceTooltip(field, evidenceByField);
      const evidenceAttrs = tooltip ? ` title="${escapeHtml(tooltip)}" aria-label="${escapeHtml(tooltip)}"` : "";
      const evidenceClass = tooltip ? " has-field-evidence" : "";
      const structuredConfig = STRUCTURED_STUDY_CHARACTERISTIC_FIELDS[field];
      const structuredClass = structuredConfig
        ? ` structured-characteristic-cell ${structuredConfig.className || ""}`
        : "";
      const cellValue = structuredConfig
        ? renderStructuredStudyCharacteristicValue(value, structuredConfig)
        : (hasValue(value) ? sentence(value) : "—");
      return `<td class="study-characteristics-value${structuredClass}${evidenceClass}"${evidenceAttrs}>${cellValue}</td>`;
    }

    function renderStructuredStudyCharacteristicValue(value, config) {
      ...
      if (arrayValue) {
        ...
        return `
          <div class="structured-group-list">
            ${arrayValue.map((item) => `
              <div class="structured-group-item">
                ${renderStructuredStudyCharacteristicObject(item, config)}
              </div>
            `).join("")}
          </div>
        `;
      }
      return renderStructuredStudyCharacteristicObject(value, config);
    }

    function renderStructuredStudyCharacteristicObject(value, config) {
      const entries = orderedObjectEntries(value, config.order || []);
      if (!entries.length) {
        return hasValue(value) ? sentence(value) : "—";
      }
      const labels = config.labels || {};
      return `
        <div class="structured-characteristic-list">
          ${entries.map(([field, itemValue]) => {
            const hasItemValue = hasValue(itemValue);
            return `
              <div class="structured-characteristic-item">
                <div class="structured-characteristic-label">${escapeHtml(labels[field] || humanizeMetric(field))}</div>
                <div class="structured-characteristic-value${hasItemValue ? "" : " is-empty"}">${hasItemValue ? sentence(itemValue) : "—"}</div>
              </div>
            `;
          }).join("")}
        </div>
    ...
    return `
      <details class="collapsible-table-panel study-characteristics-panel">
        ...
        <p class="note">Review-agnostic descriptive study information extracted once per candidate study. Showing ${rowCountText()} from the saved study-characteristics summary.</p>
        <div class="extraction-table-frame study-characteristics-frame">
          <div class="table-scroll-proxy screening-wrap" data-scroll-proxy="${escapeHtml(scrollKey)}" aria-label="Horizontal scroll for study characteristics table">
            <div class="table-scroll-proxy-inner"></div>
          </div>
          <div class="table-wrap screening-wrap study-characteristics-table-wrap" data-scroll-body="${escapeHtml(scrollKey)}">
            <table class="screening-table study-characteristics-table">
              <thead>
                <tr>
                  <th class="screen-col-index">#</th>
                  <th>PMID</th>
                  ...
                  <th>Source</th>
                  <th>Full Text</th>
                  <th class="mono">non_extraction_reason</th>
                  ${displayFields.map((field) => `<th class="mono study-characteristics-value">${escapeHtml(field)}</th>`).join("")}
                </tr>
              </thead>
              <tbody>
                ${rows.map((row, index) => `
                  <tr>
                    ...
                    <td class="screen-col-pmid mono">${renderPmidLink({ pmid: row.pmid })}</td>
                    <td class="screen-col-title">${extractionPubInfoCell(row)}</td>
                    <td>${sentence(row.characteristics_status || "—")}</td>
                    <td>${sentence(sourceLabel(row.text_source_used))}</td>
                    <td>${yesNo(row.fulltext_obtained)}</td>
                    <td class="study-characteristics-note">${sentence(row.non_extraction_reason || "—")}</td>
                    ${displayFields.map((field) => renderValueCell(row, field)).join("")}
                  </tr>
                `).join("")}
              </tbody>
            </table>
        ...
        <div class="synthesis-subgroup-list">
          ${groups.map((group) => {
            ...
            const totalRows = groupDifferenceEntries.map(([field, test]) => {
              ...
              return `
                <details class="synthesis-subgroup-total">
                  <summary class="synthesis-subgroup-summary synthesis-subgroup-total-summary">
                    <span class="synthesis-subgroup-title">Combined Subgroup Estimate</span>
            ...
            return `
              <div class="synthesis-subgroup-group">
                <div class="synthesis-subgroup-group-title">
                ...
                ${group.items.map(({ subgroup, descriptor }) => {
                  ...
                  return `
                    <details class="synthesis-subgroup-panel">
                      <summary class="synthesis-subgroup-summary">
  ...
  function finalReportSection(markdown) {
    ...
    if (!text) {
      return `
        <details class="detail-card final-report-panel" style="margin-top:14px;">
  ...
  function render() {
    ...
    const robFramework = current.rob_framework || {};
    ...
    const studyCharacteristicsSummary = current.study_characteristics_summary || {};
    ...
    renderHero(current);
	    ...
	    ${renderLeftSideKeys(publicationLinkage, publicationLinkageEvidence, cochraneSynthesisCiOverlap)}
	    ${renderEvaluationVisibilityBanner()}
	      ...
	      <div class="panel root-prompt-card">
      ...
      <div class="stats-grid" style="margin-top:14px;">
        ...
        <div class="matrix-toolbar">
	      ...
	      <div class="step-header">
	        <div>
	          <p class="kicker">Stage 4</p>
		          <h2>Outcome and review planning</h2>
		        </div>
		      </div>
	      ...
	      <div class="detail-card" style="margin-top:14px;">
	        <h3>Candidate PMC full-text retrieval</h3>
	        ${fulltextEligibilitySection(perStudyOutputs) || `<p class="note">No full-text retrieval or full-text eligibility rows were found for this run.</p>`}
	      </div>
	    </section>
	    ...
	    <section class="step-card" id="step-5">
	      ...
	      <div class="detail-card" style="margin-top:14px;">
	        ...
	        <p class="note">This section summarizes the extraction stage before showing outcome-specific rows. Study characteristics are extracted once per candidate study; outcome extraction creates one row per candidate study per configured outcome. For studies with full text, outcomes, study characteristics, and RoB are produced together. Without full text, extraction uses abstract plus exact linked NCT records when available, otherwise the abstract alone; RoB is skipped without full text.</p>
	      ...
	      ${studyCharacteristicsSection(studyCharacteristicsSummary)}
	      ${riskOfBiasTemplateSection(robFramework)}
	      ${robToolSourceSection(robDisplay)}
    ...
    if (!rowStatusDismissHandlerAttached) {
      document.addEventListener("click", (event) => {
        ...
        if (
          ...
          || event.target.closest(".cochrane-status-side-key")
